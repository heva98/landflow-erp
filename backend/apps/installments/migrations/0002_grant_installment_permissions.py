...
# Roles that create payment plans and record installment payments day to day.
FULL_ROLES = ['Managing Director', 'Finance Manager', 'Sales Manager', 'Sales Agent', 'Cashier', 'Accountant']

# Everyone else who plausibly needs to see payment plans and the customer
# ledger, view-only.
VIEWER_ROLES = [
    'CRM Officer', 'Receptionist', 'Surveyor', 'Legal Officer',
    'Marketing Officer', 'Document Officer', 'Site Manager',
]

MODELS = ['paymentplan', 'installment', 'installmentpayment']
...
def grant_permissions(apps, schema_editor):
    ...
    full_perms = Permission.objects.filter(
        content_type__app_label='installments',
...
def revoke_permissions(apps, schema_editor):
    ...
    installment_perms = Permission.objects.filter(content_type__app_label='installments')
    for role in Role.objects.filter(name__in=FULL_ROLES + VIEWER_ROLES):
        role.permissions.remove(*installment_perms)
